...
export default async function languageSwap() {
  ...
  button.addEventListener("click", () => {
    ...
    bestilleButton.forEach((element) => {
      element.textContent =
        info.utility.buttons.booking[currentLanguage].toUpperCase();
    });

    priserButton.textContent =
      info.utility.buttons.pricing[currentLanguage].toUpperCase();
    ...
    bestilleTimeTitle.textContent =
      info.booking.booking_header[currentLanguage].toUpperCase();
    ...
    if (
      todayTime.textContent === "Closed" ||
      todayTime.textContent === "Stengt"
    ) {
